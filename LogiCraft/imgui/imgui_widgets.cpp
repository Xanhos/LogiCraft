...
bool ImGui::DragFloat2(const char* label, sf::Vector2f& vec, float v_speed, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    float v[2]{ vec.x,vec.y };
    bool boolean = DragScalarN(label, ImGuiDataType_Float, v, 2, v_speed, &v_min, &v_max, format, flags);
    vec.x = v[0]; vec.y = v[1];
    return boolean;
}
...
bool ImGui::InputFloat2(const char* label, sf::Vector2f& v, const char* format, ImGuiInputTextFlags flags)
{
    float tmp[2] = { v.x, v.y };
    bool hasChange = InputScalarN(label, ImGuiDataType_Float, tmp, 2, NULL, NULL, format, flags);
    v = sf::Vector2f(tmp[0], tmp[1]);
    return hasChange;
}
...
bool ImGui::InputInt4(const char* label, sf::IntRect& v, ImGuiInputTextFlags flags)
{
    int tmp[4] = { v.left, v.top, v.width, v.height };
    bool hasChange(InputScalarN(label, ImGuiDataType_S32, tmp, 4, NULL, NULL, "%d", flags));
...
bool ImGui::InputText(const char* label, std::string& buffer, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    char* buf = new char[buf_size];
    strcpy(buf, buffer.c_str());
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    auto result = InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
    buffer = buf;
    delete[] buf;
    return result;
}
...
bool ImGui::InputTextMultiline(const char* label, std::string& buffer, size_t buf_size, const ImVec2& size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    char* buf = new char[buf_size];
    strcpy(buf, buffer.c_str());
    auto result = InputTextEx(label, NULL, buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback, user_data);
    buffer = buf;
    delete[] buf;
    return result;
}
...
IMGUI_API bool ImGui::ColorEdit4(const char* label, sf::Color& color, ImGuiColorEditFlags flags)
{
    ...
    color.a = static_cast<sf::Uint8>(col[3] * 255.f);
    return result;
}
